...
def register_client_handlers(dp: Dispatcher, models: models):
    ...
    async def callback_handlers(call: types.CallbackQuery):
        if call.data == 'assortment':
            ...
            await call.bot.send_photo(call.from_user.id, models.get_products_details()['photo'][index], f"Название: {models.get_products_details()['name'][index]}\nОписание: {models.get_products_details()['description'][index]}\nЦена товара: {models.get_products_details()['price'][index]}руб", reply_markup=types.ReplyKeyboardRemove())
            await call.message.answer(text='Что думаете?', reply_markup=kb.dinamic_product_inlain_keyboard(f"{models.get_products_details()['name'][index]} - {models.get_products_details()['price'][index]}руб"))
            await call.answer()       
